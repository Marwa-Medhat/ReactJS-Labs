...
class Todo extends React.Component{
    ...
    render(){
        return <div>
            ...
            <DisplayTasks contents={this.state.contents}  deleteContent={this.deleteContent}/><br/>
            </div>
...
class DisplayTasks extends React.Component{
    deleteContent=()=>{
        ...
        this.props.deleteContent(content);
    }
    render(){
        return <div class="list-group ">          
            {this.props.contents.length > 0 ?this.props.contents.map((item)=>{
                return <DisplayTask content={item} key={item.content} deleteContent={this.deleteContent} />
            }):"Empty"}
        </div>
    }
}
...
class DisplayTask extends React.Component{
    ...
    <button className="btn btn-success" style={{ marginLeft: 400,marginRight: 10 ,paddingRight:10}}><i className="fa fa-check "></i></button> 
